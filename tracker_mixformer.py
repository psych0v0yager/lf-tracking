"""
Robust Single Object Tracker - Version 18v2 (MixFormerV2 + Optimized Recovery)

Neural tracker using MixFormerV2-S with optimized local recovery search.

Changes from v18:
- Local recovery search instead of full-frame (faster)
- Search radius based on camera motion

Key Components:
- MixFormerV2-S: 16M param transformer tracker (~5ms on GPU)
- Camera motion compensation: Handles large camera pans
- Template verification: Fallback for drift detection
- Local pyramidal recovery: Fast re-detection in limited search area
"""
...
from trackers.mixformer_pytorch import MixFormerTracker
...
class TemplateVerifier:
    ...
    def __init__(self):
        ...
        # Edge-based templates for robustness to lighting changes
    ...
    def _compute_edges(self, img):
        ...
        if img is None or img.size == 0:
            return None
        # Use median-based adaptive thresholds
    ...
    def verify(self, gray, bbox, threshold=0.3):
        ...
        # Edge-based matching (robust to lighting changes)
        edge_score = self._verify_edge(current)

        # Conservative combination: edge only helps when gray is borderline (0.15-0.5)
        # If gray is very low (<0.15), don't trust edge alone - likely wrong target
        # If gray is good (>0.5), use gray directly
        if gray_score >= 0.5:
            combined_score = gray_score
        elif gray_score >= 0.15 and edge_score >= 0.45:
            # Borderline gray but good edge - boost more aggressively
            combined_score = gray_score + (edge_score - 0.4) * 0.4
        else:
            # Gray too low - don't trust edge, it might be wrong target
            combined_score = gray_score
    ...
    def search_light(self, gray, search_region, threshold=0.25):
        ...
        # Coarse search at 1/3 resolution (slightly higher than 1/4 for accuracy)
        down_scale = 0.33
    ...
    def search_pyramidal(self, gray, search_region, threshold=0.25):
        """Full pyramidal search: coarse at 1/4 res, refine at full res"""
        sx, sy, sw, sh = [int(v) for v in search_region]
        fh, fw = gray.shape
        sx, sy = max(0, sx), max(0, sy)
        sw, sh = min(sw, fw - sx), min(sh, fh - sy)
...
class RobustTrackerV18:
    """
    MixFormerV2-S based tracker with motion compensation.

    Key differences from V17v9:
    - Uses MixFormerV2-S neural tracker instead of CSRT
    - MixFormer provides confidence score natively
    - Faster inference (~5ms vs ~17ms for CSRT)
    - Still uses motion compensation for camera pan handling
    """
    ...
    def initialize(self, frame, bbox):
        ...
        # Create MixFormer tracker with reduced search factor to prevent drift
        print("[V18] Initializing MixFormerV2-S tracker...")
        self.tracker = MixFormerTracker(search_factor=3.0)  # Default 4.5, reduced to prevent drift

        try:
            success = self.tracker.init(frame, list(self.bbox))
            if success:
                print(f"[V18] MixFormer initialized at {self.bbox}")
            return success
        except Exception as e:
            print(f"[V18] Tracker init failed: {e}")
            return False
    ...
    def update(self, frame, timing=None):
        ...
        # Unlike CSRT, MixFormer is designed to handle large motions via search region
        # Reinitializing often causes it to lock onto wrong targets
        if timing:
            timing.start("tracker_reinit")
        # Keep track of motion for recovery decisions
        self.last_cam_motion = cam_motion
        # Store motion prediction for recovery search
        self.motion_pred_bbox = motion_pred
        if timing:
            timing.stop("tracker_reinit")

        # Run MixFormer tracker
        if timing:
            timing.start("mixformer_update")
        try:
            tracker_success, box, self.mixformer_score = self.tracker.update(frame)
        except Exception as e:
            print(f"[V18] MixFormer update error: {e}")
            tracker_success = False
            box = self.bbox
            self.mixformer_score = 0.0
        if timing:
            timing.stop("mixformer_update")
        ...
        if tracker_success and self.mixformer_score >= self.score_threshold:
            ...
            if is_valid:
                ...
                if self.mixformer_score >= 0.7:
                    # High confidence: allow 0.8-1.2 range
                    min_scale, max_scale = 0.8, 1.2
                else:
                    # Low confidence: force closer to original size (0.9-1.1)
        ...
        if not actual_success:
            ...
            # Max distance: base 150px + camera motion scaled
...
def run_tracker(video_path, initial_bbox, save_video=True, show_video=True):
    """Run the MixFormerV2 tracker on a video"""
    ...
    out = None
    if save_video:
        output_path = str(Path(video_path).stem) + "_v18_tracked.mp4"
        fourcc = cv2.VideoWriter_fourcc(*'mp4v')
        out = cv2.VideoWriter(output_path, fourcc, fps, (width, height))
        print(f"Output: {output_path}")

    tracker = RobustTrackerV18(motion_threshold=20, verify_threshold=0.50, score_threshold=0.5)
    timing = TimingStats()
    ...
    while True:
        ...
        if show_video:
            cv2.imshow("MixFormerV2 Tracker v18", frame)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="MixFormerV2 Robust Tracker v18")
