"""
Robust Single Object Tracker - Version 17v6 (NanoTrack)

Changes from v17v2:
- NanoTrack (lightweight Siamese tracker) instead of CSRT
- Much faster than CSRT (~5-10ms vs 31ms)
- Should be more robust than KCF while being faster than CSRT
"""
...
def create_tracker(backbone_path="models/nanotrack_backbone_sim.onnx",
                   head_path="models/nanotrack_head_sim.onnx"):
    """Create NanoTrack tracker"""
    params = cv2.TrackerNano_Params()
    params.backbone = backbone_path
    params.neckhead = head_path
    return cv2.TrackerNano_create(params), "NanoTrack"
...
class RobustTrackerV17v6:
    ...
    def _create_tracker(self):
        """Create a new NanoTrack tracker instance"""
        return create_tracker(self.backbone_path, self.head_path)
    ...
    def update(self, frame, timing=None):
        ...
        # Run primary tracker (NanoTrack)
        if timing:
            timing.start("nano_update")
        try:
...
def main():
    parser = argparse.ArgumentParser(description="Robust Object Tracker V17v6 (NanoTrack)")
    ...
    parser.add_argument("--backbone", default="models/nanotrack_backbone_sim.onnx", help="NanoTrack backbone model")
    parser.add_argument("--head", default="models/nanotrack_head_sim.onnx", help="NanoTrack head model")
    ...
    out_path = args.output or (Path(args.video).stem + "_tracked_v17v6.mp4")
    ...
    while True:
        ...
        # Display
        timing.start("imshow")
        cv2.imshow("Tracking V17v6", frame)
        timing.stop("imshow")
