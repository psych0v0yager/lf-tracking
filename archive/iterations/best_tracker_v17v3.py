"""
Robust Single Object Tracker - Version 17v3 (Half-Resolution CSRT)

Changes from v17v2:
- CSRT runs at 0.5x resolution (960x540 instead of 1920x1080)
- Bbox scaled down for tracker, scaled back up for output
- Expected: ~2x speedup on CSRT (31ms -> ~15ms)
- Risk: Drone at half-res is only 36x23 pixels - may lose accuracy
"""
...
class RobustTrackerV17v3:
    def __init__(self, motion_threshold=20, verify_threshold=0.5, use_yolo=True, csrt_scale=0.5):
        self.motion_threshold = motion_threshold
        self.verify_threshold = verify_threshold
        self.use_yolo = use_yolo
        self.csrt_scale = csrt_scale  # Run CSRT at this scale (0.5 = half resolution)
        ...
        # Cached small frame for CSRT
        self.small_frame = None
        self.small_frame_size = None

    def _scale_bbox_down(self, bbox):
        """Scale bbox from full resolution to CSRT resolution"""
        x, y, w, h = bbox
        return (
            int(x * self.csrt_scale),
            int(y * self.csrt_scale),
            max(1, int(w * self.csrt_scale)),
            max(1, int(h * self.csrt_scale))
        )

    def _scale_bbox_up(self, bbox):
        """Scale bbox from CSRT resolution back to full resolution"""
        x, y, w, h = bbox
        return (
            int(x / self.csrt_scale),
            int(y / self.csrt_scale),
            int(w / self.csrt_scale),
            int(h / self.csrt_scale)
        )

    def _get_small_frame(self, frame):
        """Get downscaled frame for CSRT"""
        fh, fw = frame.shape[:2]
        new_w = int(fw * self.csrt_scale)
        new_h = int(fh * self.csrt_scale)
        if self.small_frame_size != (new_w, new_h):
            self.small_frame_size = (new_w, new_h)
    ...
    def initialize(self, frame, bbox):
        ...
        # Initialize tracker on small frame
        small_frame = self._get_small_frame(frame)
        small_bbox = self._scale_bbox_down(self.bbox)

        print(f"CSRT scale: {self.csrt_scale} ({frame.shape[1]}x{frame.shape[0]} -> {small_frame.shape[1]}x{small_frame.shape[0]})")
        print(f"Bbox: {self.bbox} -> {small_bbox} (drone: {small_bbox[2]}x{small_bbox[3]} px)")
    ...
    def update(self, frame, timing=None):
        ...
        # Create small frame for CSRT (do this once per frame)
        if timing:
            timing.start("frame_resize")
        small_frame = self._get_small_frame(frame)
        if timing:
            timing.stop("frame_resize")
        ...
        if cam_motion > self.motion_threshold:
            ...
            small_motion_pred = self._scale_bbox_down(motion_pred)
            self.tracker.init(small_frame, small_motion_pred)
        if timing:
            timing.stop("tracker_reinit")

        # Run primary tracker (CSRT) on small frame
        if timing:
            timing.start("csrt_update")
        try:
            tracker_success, small_box = self.tracker.update(small_frame)
            # Scale box back to full resolution
            if tracker_success:
                box = self._scale_bbox_up(tuple(small_box))
            else:
                box = self.bbox
        except:
...
def main():
    parser = argparse.ArgumentParser(description="Robust Object Tracker V17v3 (Half-Resolution CSRT)")
    ...
    parser.add_argument("--csrt-scale", type=float, default=0.5, help="CSRT resolution scale (0.5 = half)")
    ...
    tracker = RobustTrackerV17v3(
        verify_threshold=args.verify_threshold,
        use_yolo=not args.no_yolo,
        csrt_scale=args.csrt_scale
    )
    ...
    out_path = args.output or (Path(args.video).stem + "_tracked_v17v3.mp4")
    ...
    while True:
        ...
        # Display
        timing.start("imshow")
        cv2.imshow("Tracking V17v3", frame)
        timing.stop("imshow")
