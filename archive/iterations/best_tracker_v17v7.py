"""
Robust Single Object Tracker - Version 17v7 (Fast Recovery)

Changes from v17v2:
- Removed YOLO (was loaded but never used)
- Pyramidal recovery: search at 1/4 res first, refine at full res
- Fewer scales: [0.9, 1.0, 1.1] instead of 5 scales
- Expected: recovery 500ms -> ~50-100ms
"""
...
def main():
    parser = argparse.ArgumentParser(description="Robust Object Tracker V17v7 (Fast Recovery)")
    ...
    writer = None
    if not args.no_save:
        out_path = args.output or (Path(args.video).stem + "_tracked_v17v7.mp4")
    ...
    while True:
        ...
        if not args.no_show:
            timing.start("imshow")
            cv2.imshow("Tracking V17v7", frame)
            timing.stop("imshow")
