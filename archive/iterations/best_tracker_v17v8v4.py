"""
Robust Single Object Tracker - Version 17v8v4 (Fast CSRT + Drift Detection)

Changes from v17v8v4:
- Add PROACTIVE drift detection during tracking (not just recovery)
- If tracker output diverges from motion prediction by >60px AND score < 0.60,
  reject tracker output and trigger recovery
- This catches drift BEFORE it locks onto wrong target
"""
...
def create_tracker():
    """Create CSRT tracker with speed-optimized parameters"""
    params = cv2.TrackerCSRT_Params()

    # Biggest wins - disable expensive features
    params.use_segmentation = False      # Saves ~5-10ms (most expensive!)
    params.number_of_scales = 17         # Default 33, fewer scale tests
    params.template_size = 150           # Default 200, smaller template
    params.padding = 2.5                 # Default 3.0, smaller search area

    # Moderate wins
    params.admm_iterations = 3           # Default 4, fewer filter iterations
    params.histogram_bins = 8            # Default 16, coarser color histogram

    return cv2.TrackerCSRT_create(params), "CSRT-fast"
...
class TemplateVerifier:
    ...
    def _search_with_template_fast(self, gray, search_region, template, threshold, scales=[0.9, 1.0, 1.1]):
        """Fast search using a specific template with fewer scales"""
        if template is None:
            return None
    ...
    def search_pyramidal(self, gray, search_region, threshold=0.25):
        """
        Fast pyramidal search: coarse search at 1/4 res, refine at full res.
        ~5-10x faster than full-frame full-res search.
        """
        sx, sy, sw, sh = [int(v) for v in search_region]
        fh, fw = gray.shape
        sx, sy = max(0, sx), max(0, sy)
        ...
        # Step 1: Coarse search at 1/4 resolution
        down_scale = 0.25
        ...
        # Coarse search with just scale 1.0 (fastest)
        coarse_match = self._search_with_template_fast(gray_small, search_small, template_small,
                                                       threshold * 0.8, scales=[1.0])

        if coarse_match is None:
            # Try original template at coarse level
            orig_small = cv2.resize(self.original_template, None, fx=down_scale, fy=down_scale, interpolation=cv2.INTER_AREA)
            coarse_match = self._search_with_template_fast(gray_small, search_small, orig_small,
                                                           threshold * 0.8, scales=[1.0])
        ...
        cx, cy, cw, ch, coarse_score = coarse_match

        # Scale back to full res and create refinement region (3x the template size)
        full_x = int(cx / down_scale)
        full_y = int(cy / down_scale)
        th, tw = self.reference_template.shape[:2]

        margin = max(tw, th) * 2
        refine_x = max(0, full_x - margin)
        refine_y = max(0, full_y - margin)
        refine_w = min(tw + margin * 3, fw - refine_x)
        refine_h = min(th + margin * 3, fh - refine_y)

        refine_region = (refine_x, refine_y, refine_w, refine_h)

        # Fine search with 3 scales
        match1 = self._search_with_template_fast(gray, refine_region, self.reference_template,
                                                  threshold, scales=[0.9, 1.0, 1.1])
        match2 = self._search_with_template_fast(gray, refine_region, self.original_template,
                                                  threshold, scales=[0.9, 1.0, 1.1])

        # Return best match
        if match1 is None and match2 is None:
            # Fall back to coarse result scaled up
            return (full_x, full_y, tw, th, coarse_score)
        if match1 is None:
            return match2
        if match2 is None:
            return match1
    ...
    def search(self, gray, search_region, threshold=0.25):
        """Search using pyramidal approach for speed"""
        return self.search_pyramidal(gray, search_region, threshold)


class RobustTrackerV17v8v3:
    def __init__(self, motion_threshold=20, verify_threshold=0.5):
    ...
    def update(self, frame, timing=None):
        ...
        # Run primary tracker (CSRT)
        if timing:
            timing.start("csrt_update")
        try:
        ...
        if tracker_success:
            ...
            # DRIFT DETECTION: Check if tracker diverged from motion prediction
            # If tracker is far from prediction AND score is borderline, reject
            if is_valid and self.verify_score < 0.60:
                tracker_cx = temp_bbox[0] + temp_bbox[2] / 2
                tracker_cy = temp_bbox[1] + temp_bbox[3] / 2
                pred_cx = motion_pred[0] + motion_pred[2] / 2
                pred_cy = motion_pred[1] + motion_pred[3] / 2
                drift_dist = np.sqrt((tracker_cx - pred_cx)**2 + (tracker_cy - pred_cy)**2)
                if drift_dist > 60:  # More than 60px from prediction with weak score
                    is_valid = False

            if is_valid:
                x, y, w, h = temp_bbox
                orig_w, orig_h = self.original_size
                w = max(int(orig_w * 0.8), min(int(orig_w * 1.2), w))
                h = max(int(orig_h * 0.8), min(int(orig_h * 1.2), h))
        ...
        if not actual_success:
            ...
            # Helper to check if recovery position is reasonable
            def is_valid_recovery(mx, my):
                if self.last_good_bbox is None:
                    return True
                lx, ly, lw, lh = self.last_good_bbox
                last_cx, last_cy = lx + lw/2, ly + lh/2
                new_cx, new_cy = mx + orig_w/2, my + orig_h/2
                dist = np.sqrt((new_cx - last_cx)**2 + (new_cy - last_cy)**2)
                return dist < 300  # Max 300px from last known position

            # IMMEDIATE full-frame search with HIGH threshold (stricter than v17v8)
            match = self.verifier.search(gray, (0, 0, fw, fh), threshold=0.60)

            if match:
                mx, my, mw, mh, score = match
                if is_valid_recovery(mx, my):
            ...
            # If high-threshold search fails, try medium threshold with distance check
            if not actual_success:
                match = self.verifier.search(gray, (0, 0, fw, fh), threshold=0.50)

                if match:
                    mx, my, mw, mh, score = match
                    # Must pass both score AND distance checks
                    if score > 0.55 and is_valid_recovery(mx, my):
                        self.bbox = clamp_bbox((mx, my, orig_w, orig_h), frame.shape)
                        self.verify_score = score
...
def main():
    parser = argparse.ArgumentParser(description="Robust Object Tracker V17v8v3 (Fast + Strict Recovery)")
    ...
    parser.add_argument("--no-save", action="store_true", help="Disable video output (faster)")
    parser.add_argument("--no-show", action="store_true", help="Disable display window (faster)")
    ...
    print(f"Show display: {not args.no_show}")
    if not args.no_show:
    ...
    writer = None
    if not args.no_save:
        out_path = args.output or (Path(args.video).stem + "_tracked_v17v8v4.mp4")
    ...
    while True:
        ...
        if not args.no_show:
            timing.start("imshow")
            cv2.imshow("Tracking V17v8v3", frame)
            timing.stop("imshow")
