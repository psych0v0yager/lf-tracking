"""
Robust Single Object Tracker - Version 16v4

Optimized for speed while keeping accuracy:
- Motion estimation: Optimized (from v16v3)
- CSRT with tuned parameters (faster than default, more accurate than KCF)
- Faster recovery with limited search region first

Usage:
    python best_tracker_v16v4.py test_track_dron1.mp4 1314 623 73 46
"""
...
def create_tracker():
    # CSRT with tuned parameters for speed
    params = cv2.TrackerCSRT_Params()

    # Reduce template size for speed (default is adaptive)
    params.template_size = 150  # Smaller template = faster

    # Disable expensive features
    params.use_hog = True       # Keep HOG (important for accuracy)
    params.use_color_names = False  # Disable color names (expensive)
    params.use_gray = True      # Keep grayscale
    params.use_rgb = False      # Disable RGB features
    params.use_channel_weights = False  # Disable channel weighting
    params.use_segmentation = False     # Disable segmentation mask

    # Reduce histogram bins
    params.histogram_bins = 8   # Default is 16
    params.number_of_scales = 33  # Default is 33, could reduce but affects accuracy
    params.scale_step = 1.02    # Default is 1.02

    # Faster learning
    params.background_ratio = 2  # Default is 2
    params.histogram_lr = 0.04   # Default is 0.04

    tracker = cv2.TrackerCSRT_create(params)
    return tracker, "CSRT-Fast"


class CameraMotionEstimator:
    def __init__(self, scale=0.5):
        self.prev_gray_small = None
        self.scale = scale  # Process at half resolution
        self.lk_params = dict(
            winSize=(15, 15),  # Reduced from 21x21
            maxLevel=2,        # Reduced from 4
            criteria=(cv2.TERM_CRITERIA_EPS | cv2.TERM_CRITERIA_COUNT, 20, 0.03)  # Relaxed criteria
        )
        self.feature_params = dict(
            maxCorners=100,    # Reduced from 300
            qualityLevel=0.02, # Slightly higher quality threshold
            minDistance=15,    # Reduced from 20
            blockSize=5        # Reduced from 7
        )
    ...
    def update(self, gray, bbox):
        ...
        # Scale bbox to small image coordinates
        x, y, bw, bh = [int(v * self.scale) for v in bbox]
        margin = max(15, bw, bh)
        x1, y1 = max(0, x - margin), max(0, y - margin)
        x2, y2 = min(small_w, x + bw + margin), min(small_h, y + bh + margin)

        mask = np.ones((small_h, small_w), dtype=np.uint8) * 255
        mask[y1:y2, x1:x2] = 0

        prev_pts = cv2.goodFeaturesToTrack(self.prev_gray_small, mask=mask, **self.feature_params)
        ...
        curr_pts, status, _ = cv2.calcOpticalFlowPyrLK(
            self.prev_gray_small, gray_small, prev_pts, None, **self.lk_params
        )
        ...
        # Scale points back to full resolution for homography
        good_prev_full = good_prev / self.scale
        good_curr_full = good_curr / self.scale

        H, inliers = cv2.findHomography(good_prev_full, good_curr_full, cv2.RANSAC, 5.0)
        ...
        if inliers is not None:
            ...
            if inlier_mask.sum() > 0:
                motion = np.median(np.linalg.norm(
                    good_curr_full[inlier_mask] - good_prev_full[inlier_mask], axis=1
                ))
...
class TemplateVerifier:
    ...
    def _search_with_template(self, gray, search_region, template, threshold):
        ...
        best_loc = None
        best_scale = 1.0
        for scale in [0.9, 1.0, 1.1]:  # Reduced from 5 to 3 scales
...
class RobustTrackerV16:
    ...
    def update(self, frame, timing=None):
        ...
        # Recovery - optimized to search local region first
        if not actual_success:
            ...
            # Step 1: Search LOCAL region around last good position (fast)
            if self.last_good_bbox is not None:
                lg_cx = self.last_good_bbox[0] + orig_w / 2
                lg_cy = self.last_good_bbox[1] + orig_h / 2

                # Expand search based on lost count
                expand = 4.0 + self.lost_count * 0.5
                search_w = int(orig_w * expand)
                search_h = int(orig_h * expand)
                ...
                match = self.verifier.search(gray, local_region, threshold=0.50)
            ...
            # Step 2: Full-frame search only if local failed (slow but thorough)
            if not actual_success:
...
def main():
    parser = argparse.ArgumentParser(description="Robust Object Tracker V16v4 - CSRT-Fast")
    ...
    out_path = args.output or (Path(args.video).stem + "_tracked_v16v4.mp4")
    ...
    while True:
        ...
        # Display
        timing.start("imshow")
        cv2.imshow("Tracking V16v4", frame)
        timing.stop("imshow")
