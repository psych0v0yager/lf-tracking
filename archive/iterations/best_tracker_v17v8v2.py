"""
Robust Single Object Tracker - Version 17v8v2 (Fast CSRT + Drift Compensation)

Changes from v17v8v2:
- Keep use_segmentation = False for speed (~67 FPS)
- Add drift compensation to maintain accuracy:
  - Stricter verify threshold: 0.55 (was 0.50)
  - Faster score drop detection: 0.15 (was 0.25)
  - Tighter size constraints: 80-120% (was 70-130%)
  - Lower motion threshold: 15px (was 20px) for more frequent reinit
"""
...
class RobustTrackerV17v8v2:
    def __init__(self, motion_threshold=15, verify_threshold=0.55):
    ...
    def update(self, frame, timing=None):
        ...
        if tracker_success:
            ...
            if len(self.score_history) >= 3:
                ...
                # If score drops more than 0.15 from recent average, reject (stricter)
                if self.verify_score < recent_avg - 0.15:
                    is_valid = False
...
def main():
    parser = argparse.ArgumentParser(description="Robust Object Tracker V17v8v2 (Optimized CSRT)")
    ...
    parser.add_argument("--verify-threshold", type=float, default=0.55)
    ...
    writer = None
    if not args.no_save:
        out_path = args.output or (Path(args.video).stem + "_tracked_v17v8v2.mp4")
    ...
    while True:
        ...
        if not args.no_show:
            timing.start("imshow")
            cv2.imshow("Tracking V17v8v2", frame)
            timing.stop("imshow")
