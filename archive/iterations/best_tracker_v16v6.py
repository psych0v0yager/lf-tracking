"""
Robust Single Object Tracker - Version 16v6

Full resolution motion estimation with reduced parameters:
- Motion estimation: Full res, fewer points (150 vs 300), smaller window
- CSRT: Default parameters (accurate)
- Recovery: Local search first (faster)

Expected: ~15-20 FPS with full accuracy

Usage:
    python best_tracker_v16v6.py test_track_dron1.mp4 1314 623 73 46
"""
...
class CameraMotionEstimator:
    def __init__(self):
        self.prev_gray = None
        # Optimized but full resolution
        self.lk_params = dict(
            winSize=(15, 15),  # Reduced from 21x21
            maxLevel=3,        # Reduced from 4
            criteria=(cv2.TERM_CRITERIA_EPS | cv2.TERM_CRITERIA_COUNT, 25, 0.01)
        )
        self.feature_params = dict(
            maxCorners=150,    # Reduced from 300
            qualityLevel=0.01, # Same as original
            minDistance=20,    # Same as original
            blockSize=7        # Same as original
        )
...
def main():
    parser = argparse.ArgumentParser(description="Robust Object Tracker V16v6 - FullRes")
    ...
    out_path = args.output or (Path(args.video).stem + "_tracked_v16v6.mp4")
    ...
    while True:
        ...
        # Display
        timing.start("imshow")
        cv2.imshow("Tracking V16v6", frame)
        timing.stop("imshow")
