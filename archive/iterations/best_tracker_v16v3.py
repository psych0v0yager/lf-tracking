"""
Robust Single Object Tracker - Version 16v3

Optimized for speed:
- Motion estimation: 100 points (was 300), 2 pyramid levels (was 4), 15x15 window (was 21x21)
- Half-resolution motion estimation
- KCF tracker instead of CSRT (5-10x faster)
- Still includes timing for benchmarking

Usage:
    python best_tracker_v16v3.py test_track_dron1.mp4 1314 623 73 46
"""
...
def create_tracker():
    # KCF is much faster than CSRT (5-10x) with acceptable accuracy
...
class RobustTrackerV16:
    ...
    def update(self, frame, timing=None):
        ...
        # Run primary tracker (KCF)
        if timing:
            timing.start("kcf_update")
        try:
...
def main():
    parser = argparse.ArgumentParser(description="Robust Object Tracker V16v3 - Optimized")
    ...
    out_path = args.output or (Path(args.video).stem + "_tracked_v16v3.mp4")
    ...
    while True:
        ...
        # Display
        timing.start("imshow")
        cv2.imshow("Tracking V16v3", frame)
        timing.stop("imshow")
