"""
Robust Single Object Tracker - Version 8

Key insight from frame analysis:
- Motion compensation OVERSHOOTS when camera catches up to drone
- Because drone is moving in same direction as camera pan
- So drone moves LESS than background, but we predict it moves WITH background

Fix: Don't reinitialize during high camera motion. Let CSRT handle it.
Only intervene when CSRT actually reports failure.

Usage:
    python best_tracker_v8.py test_track_dron1.mp4 1314 623 73 46
"""
...
class RobustTrackerV8:
    """
    Key change from previous versions:
    - DON'T reinitialize tracker during high camera motion
    - Only use motion compensation for RECOVERY, not during normal tracking
    - Let CSRT handle camera motion on its own
    """
    ...
    def __init__(self, motion_threshold=20):
        ...
        # Keep last known good positions for recovery
    ...
    def update(self, frame):
        ...
        # Get camera motion (for info display and recovery, NOT for reinitialization)
        H, cam_motion = self.camera_motion.update(gray, self.bbox)

        # KEY CHANGE: Do NOT reinitialize during high motion!
        # Just let CSRT handle it. CSRT is designed to handle motion.
        ...
        if success:
            ...
            self.position_history.append(self.bbox)
            ...
            # RECOVERY: Now we use motion compensation, but carefully

            # Strategy 1: Search around last good position (most reliable)
            # Don't trust motion compensation during recovery - it's what got us here
            if self.last_good_bbox is not None:
                ...
                if match:
                    ...
                    # Reinitialize tracker at found position
            ...
            # Strategy 2: If local search failed, try motion-compensated position
            if self.recovery_method is None and H is not None:
                try:
                    cam_pred = transform_bbox(self.last_good_bbox or self.bbox, H, frame.shape)
            ...
            # Last resort: just use last known position
            if self.recovery_method is None:
                if self.last_good_bbox is not None:
...
def main():
    parser = argparse.ArgumentParser(description="Robust Object Tracker V8")
    ...
    tracker = RobustTrackerV8()
    if not tracker.initialize(frame, bbox):
        print("Error: Failed to initialize tracker")
        sys.exit(1)

    print(f"\nTracker: {tracker.tracker_name}")
    print(f"V8: No motion reinit, recovery-only motion comp")
    print("\nPress 'q' to quit, SPACE to pause\n")

    out_path = args.output or (Path(args.video).stem + "_tracked_v8.mp4")
    ...
    while True:
        ...
        frame_num += 1
        success, bbox, cam_motion = tracker.update(frame)
        ...
        cv2.putText(frame, f"Frame {frame_num} | {status} | CamMotion: {cam_motion:.1f}px",
                    (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)
        cv2.putText(frame, f"V8: CSRT only, no motion reinit",
                    (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 0), 2)

        writer.write(frame)
        cv2.imshow("Tracking V8", frame)
