"""
Robust Single Object Tracker - Version 16v8

Tests faster motion estimation algorithms:
- Phase correlation (FFT-based, very fast ~5ms)
- ORB features + matching (medium ~15ms)
- Original optical flow (accurate ~60ms)

Usage:
    python best_tracker_v16v8.py test_track_dron1.mp4 1314 623 73 46
    python best_tracker_v16v8.py test_track_dron1.mp4 1314 623 73 46 --motion-method phase
    python best_tracker_v16v8.py test_track_dron1.mp4 1314 623 73 46 --motion-method orb
    python best_tracker_v16v8.py test_track_dron1.mp4 1314 623 73 46 --motion-method optflow
"""
...
class CameraMotionEstimator:
    """
    Supports multiple motion estimation methods:
    - 'phase': Phase correlation (FFT-based, very fast)
    - 'orb': ORB features + matching (medium speed)
    - 'optflow': Sparse optical flow (accurate but slow)
    - 'cuda': CUDA optical flow (fast + accurate, requires CUDA OpenCV)
    """
    ...
    def __init__(self, method='optflow'):
        ...
        try:
            if cv2.cuda.getCudaEnabledDeviceCount() > 0:
                ...
                self.gpu_flow = cv2.cuda.SparsePyrLKOpticalFlow_create(
                    winSize=(21, 21),
                    maxLevel=4
                )
                print("CUDA optical flow initialized!")
        except:
            pass

    def _phase_correlation(self, gray, bbox):
        """Fast FFT-based global translation estimation"""
        ...
        scale = 0.5
        h, w = gray.shape
        small_prev = cv2.resize(self.prev_gray, (int(w*scale), int(h*scale)))
        small_curr = cv2.resize(gray, (int(w*scale), int(h*scale)))

        # Phase correlation
        shift, response = cv2.phaseCorrelate(
            small_prev.astype(np.float64),
            small_curr.astype(np.float64)
        )
        ...
        # Create translation matrix (not full homography)
        H = np.array([
            [1, 0, dx],
            [0, 1, dy],
            [0, 0, 1]
    ...
    def _orb_matching(self, gray, bbox):
        """ORB feature matching for motion estimation"""
        if self.prev_gray is None:
            self.prev_gray = gray.copy()
            return None, 0
        ...
        if inliers is not None:
            ...
            if inlier_mask.sum() > 0:
                pts1_flat = pts1.reshape(-1, 2)[inlier_mask]
                pts2_flat = pts2.reshape(-1, 2)[inlier_mask]
                motion = np.median(np.linalg.norm(pts2_flat - pts1_flat, axis=1))

        self.prev_gray = gray.copy()
        return H, motion

    def _optical_flow(self, gray, bbox):
        """Original sparse optical flow method"""
        if self.prev_gray is None:
            self.prev_gray = gray.copy()
            return None, 0
    ...
    def _cuda_optical_flow(self, gray, bbox):
        """CUDA-accelerated sparse optical flow"""
        if not self.cuda_available:
            return self._optical_flow(gray, bbox)

        if self.prev_gray is None:
            self.prev_gray = gray.copy()
            self.gpu_prev = cv2.cuda_GpuMat()
            self.gpu_prev.upload(gray)
            return None, 0
        ...
        # Detect features on CPU (faster for small number of features)
        prev_pts = cv2.goodFeaturesToTrack(self.prev_gray, mask=mask, **self.feature_params)

        if prev_pts is None or len(prev_pts) < 8:
            self.prev_gray = gray.copy()
            self.gpu_prev = gpu_curr
            return None, 0

        # Upload points and compute flow on GPU
        gpu_prev_pts = cv2.cuda_GpuMat()
        gpu_prev_pts.upload(prev_pts)

        gpu_curr_pts, gpu_status = self.gpu_flow.calc(
            self.gpu_prev, gpu_curr, gpu_prev_pts, None
        )
    ...
    def update(self, gray, bbox):
        """Run motion estimation with selected method"""
        if self.method == 'phase':
            return self._phase_correlation(gray, bbox)
        elif self.method == 'orb':
            return self._orb_matching(gray, bbox)
        elif self.method == 'cuda':
            return self._cuda_optical_flow(gray, bbox)
        else:  # 'optflow'
            return self._optical_flow(gray, bbox)
...
class RobustTrackerV16:
    def __init__(self, motion_threshold=20, verify_threshold=0.5, use_yolo=True, motion_method='optflow'):
        ...
        self.camera_motion = CameraMotionEstimator(method=motion_method)
...
def main():
    parser = argparse.ArgumentParser(description="Robust Object Tracker V16v8 - Multiple Motion Methods")
    ...
    parser.add_argument("--motion-method", "-m", choices=['phase', 'orb', 'optflow', 'cuda'],
                        default='optflow', help="Motion estimation method")
    ...
    print(f"Motion method: {args.motion_method}")
    ...
    print(f"Motion: {args.motion_method}")
    print(f"YOLO recovery: {'enabled' if tracker.use_yolo else 'disabled'}")
    print("\nPress 'q' to quit, SPACE to pause\n")

    out_path = args.output or (Path(args.video).stem + f"_tracked_v16v8_{args.motion_method}.mp4")
    ...
    while True:
        ...
        # Display
        timing.start("imshow")
        cv2.imshow(f"Tracking V16v8 ({tracker.motion_method})", frame)
        timing.stop("imshow")
